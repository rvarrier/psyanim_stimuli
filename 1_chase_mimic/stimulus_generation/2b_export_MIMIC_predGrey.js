...
import dataStr from "/Users/f0053cz/Documents/GitHub/chase_mimic_study/test_version/base_stim_mimic_predGrey.psyn";
//import dataStr from "/Users/f0053cz/Documents/psyanim_notapp/psyanim_notapp_new_mimic_expt/test_version/base_stim_mimic_predGrey.psyn";
...
let exportOptions = {
  ...
  //exportPath: "/Users/f0053cz/Documents/psyanim_notapp/psyanim_notapp_new_mimic_expt/test_version/vids/mimic/mimic_flipped_.webm",
  variationGroups: [],
  frameRate: 30,
  motionBlurFrames: 0,
  quality: 1,
...
console.log('nr. of start positions')
const start_ind = nvals / 2; //36
for (let i = start_ind; i < nvals; i++) {
  ...
  for (let var_j of posPredatorMimicVariations) { // pred start position left or right
    ...
    { // 3 variants with different start position
      ...
      if (condition == 'redo_bad')
       {
        ...
        if (bad_inds_obj.grey.find(element => element == pos_ind)) {
          export_vid = true;
      ...
      if (export_vid == true)
      {
        ...
        let flname = "pred_subt" + var_i[0].val + '_Grey_Pos' + pred_xpos.param[last_ind] + "_" + pred_xpos.val + "_InvisPreyLocInd_" + pos_ind
        console.log(flname)
