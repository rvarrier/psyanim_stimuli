...
import dataStr from "/Users/f0053cz/Documents/GitHub/chase_mimic_study/stimulus_generation/base_stim_chase_predGrey.psyn";
...
let condition = 'redo_bad'; //'redo_bad','all'
var export_path;
...
if (condition == 'all') {
  //console.log('in all');
  export_path = "/Users/f0053cz/Documents/GitHub/chase_mimic_study/stimulus_generation/vids/chase3/chase_.webm";
}
...
if (condition == 'redo_bad') {
  //console.log('in')
  //bad_root_filenames = ['pred_subt60_Grey_Posx_350', 'pred_subt0_Black_Posx_850']
  bad_root_filenames = ['chase_pred_subt60_Grey_Posx_350_var1']
}
...
for (let var_i of subtletyVariations) { //0,30,...150
  for (let var_j of posPredatorPreyVariations) {// LR or RL
    for (let var_k = 0; var_k < 3; var_k++) {// recollapses
      ...
      let flname = "pred_subt" + var_i[0].val + '_Grey_Pos' + pred_xpos.param[last_ind] + "_" + pred_xpos.val + "_var" + var_k
      console.log(flname)
